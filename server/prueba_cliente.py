...
def revisar_forma(estado):
    ...
    if len(estado["paredes"]) != 192:
        problemas.append("paredes tiene " + str(len(estado["paredes"])) +
                         " y deben ser 192")
    if len(estado["bomberos"]) != 6:
        problemas.append("hay " + str(len(estado["bomberos"])) +
                         " bomberos y deben ser 6")
...
if len(problemas) == 0:
    print()
    print("El JSON trae la forma del contrato:")
    print("48 fuego, 48 poi, 192 paredes, 6 bomberos.")
else:
