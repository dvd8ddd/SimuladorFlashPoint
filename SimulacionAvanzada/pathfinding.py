...
# puede_pasar si hay una pared de por medio. Las puertas cerradas si cuentan
# como vecinas porque el bombero las puede abrir.
import heapq
...
def get_neighborhood(model, position):
    ...
    for direccion in range(4):
        ...
        elif model.paredes[row][col][direccion] == 4:
            vecina = celda_vecina(row, col, direccion)
            if vecina is not None:
                result.append(vecina)      # cerrada, pero abrirla cuesta un solo ap
    return result
