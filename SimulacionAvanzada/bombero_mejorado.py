# Estrategia mejorada: roles que cambian segun como va el incendio y rutas con
# dijkstra que si toman en cuenta las puertas ahora si xd
# Hereda del Bombero de la simulacion aleatoria y solo cambia el step(), o sea
# solo cambia como decide. Las acciones (mover, apagar, cargar, dejar, abrir una
# puerta) son las mismas y con las mismas reglas del juego.
...
class BomberoMejorado(Bombero):
    def __init__(self, model):
        ...
        self.papel = "rescate"        # el papel de esta ronda, puede cambiar
        self.papel_base = "rescate"   # al que regresa cuando el fuego ya bajo
        self.objetivo = None       # a que celda va, para no ir todos a la misma

    def step(self):
        self.papel = self.papel_base
        if self.papel == "rescate" and not self.cargando:
            if self.contar_llamas() > UMBRAL_LLAMAS:
                self.papel = "contencion"   # con el edificio asi no hay a quien sacar
    ...
    def avanzar(self):
        ...
        siguiente = camino[1]
        direccion = self.direccion_hacia(siguiente)
        if direccion >= 0 and self.model.paredes[fila][col][direccion] == 4:
            # sin abrirla ese cuarto no existe para el bombero
            return self.abrir_cerrar_puerta(direccion)

        return self.mover(siguiente[0], siguiente[1])

    def direccion_hacia(self, celda):
        (col, fila) = self.pos
        for direccion in range(4):
            if celda_vecina(fila, col, direccion) == celda:
                return direccion
        return -1
...
UMBRAL_LLAMAS = 8    # arriba de esto los rescatistas sueltan y se van a apagar
